...
function getPreambleRange (editor, row) {
  ...
  if (endPoint === null) {
    endPoint = scanRange.end;
  }

  // TODO: Expand any folds hiding the \begin{document}

  const preambleRange = new Range(startPoint, endPoint);

  if (!atom.config.get("latex-folding.foldTrailingPreambleWhitespace")) {
    translateEndRowToLastNonWhitespaceLine(preambleRange, editor);
  }
...
function getSectionRange (editor, row, sectionType) {
  ...
  editor.scanInBufferRange(searchRegex, scanRange, ({match, range, stop}) => {
    ...
    if (command && startLevel < levelTable[command]) { return; }
